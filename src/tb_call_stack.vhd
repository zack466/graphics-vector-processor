...
architecture sim of tb_call_stack is
    ...
    signal mem_stall       : std_logic := '0'; -- never stall in this test
...
begin
    ...
    process
        ...
        constant EXPECTED_PIXEL  : std_logic_vector(31 downto 0) := x"42424242";
    ...
    begin
        ...
        -- (C) Check all 32 thread pixels
        -- pixel_buf_data[i*32+31 : i*32] = packed pixel for thread i
        for i in 0 to WARP_SIZE-1 loop
            if pixel_buf_data(i*32+31 downto i*32) /= EXPECTED_PIXEL then
                report "FAIL (C): thread " & integer'image(i) &
                       " pixel: expected 0x42424242, got 0x" &
                       to_hstring(pixel_buf_data(i*32+31 downto i*32))
                    severity error;
                all_pixels_ok := false;
            end if;
        end loop;
        assert all_pixels_ok report "FAIL (C): pixel mismatch in one or more threads"
            severity failure;
        report "PASS (C): all 32 thread pixels = 0x42424242";

        -- ----------------------------------------------------------------
        -- 5. mem_stall is already '0'; MEM_WAIT releases immediately.
        -- ----------------------------------------------------------------
        wait until rising_edge(clk);
