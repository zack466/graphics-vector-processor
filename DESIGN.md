...
The top-level entity is `frame_processor.vhd`, a structural entity (no datapath logic) that wires together:
* **`warp_scheduler.vhd`** — frame-level FSM that iterates `warp_offset` from 0 to `frame_width × frame_height − 1` in steps of 32, dispatching one warp block at a time.
* **`warp_unit.vhd`** — self-contained SIMT warp: IFU, decode, issue, FPU/ALU/RED execution, VRF, PRF, and pixel snoop buffer. Accepts `warp_start`/`warp_offset`; exposes `pixel_buf_valid`, `pixel_buf_data` (1024-bit flat buffer of 32 packed RGBA pixels), and `warp_halted`.
* **`mcu_block_transfer.vhd`** — accepts the warp's 1024-bit pixel buffer and emits 8 sequential 128-bit Avalon burst beats to DDR3.
* **`avm_burst_bridge.vhd`** — thin Avalon-MM master protocol layer between the MCU and the DDR3 controller.
* **`instruction_memory.vhd`** — shared M10K BRAM; all warps execute the same shader program.
* **`pixel_buffer_ram.vhd`** — M10K BRAM that stores a single warp's 32 output pixels, to be written to memory by the MCU

The host drives `frame_start` (1-cycle pulse), `frame_width`, and `frame_height` and waits for `frame_done`.
...
The processor operates as an accelerator co-processor, managed by an external host.
A Quartus top-level is provided in `top_level.vhd` for integration with Platform Designer.

**2.1. IMEM Programming Interface** (`instruction_memory.vhd`)
...
All datapaths are combined in a single multiplexed unit in `execution_unit.vhd`.

**3.1. Standard FPU Lanes (4x Independent)** (`fpu_lane.vhd`)
...
**3.2. Dedicated Integer ALU Lane** (`alu_lane.vhd`)
...
* **Pipeline Synchronization:** While integer math evaluates combinationally in zero cycles, the hardware injects the result into a shift register. This synchronizes the ALU output perfectly with the unified Writeback Controller.

**3.3. Parallel Vector Reduction Unit** (`vector_reduction_unit.vhd`)
A dedicated 16-cycle Altera floating-point 4D scalar product block handles cross-coordinate math (Dot Products, Squared Magnitudes, Sums). Dynamic input masking allows the unit to switch between 3D and 4D operations without latency penalties.
...
Instruction decoding constants can be found in `processor_constants_pkg.vhd`, and the actual decoding logic in `instruction_decoder.vhd`.

**4.1. Hardware Modifiers (Logic & Math)**
* **Dual-Port Swizzle:** Combinational crossbar routing for operands. To reduce FPGA routing pressure, this currently supports passthrough (`.xyzw`) and scalar broadcasting/splatting (`.xxxx`, `.yyyy`, `.zzzz`, `.wwww`). Arbitrary cross-component swizzling has been removed but could be added later via a dedicated instruction. Implemented in `swizzle_network.vhd`.
...
This is fully managed in `instruction_fetch_unit.vhd`.
...
2. The Execution Unit tracks this token down a shift register.
3. The IFU stalls in the `EXEC_WAIT` state until `exec_flush_active` drops to `0`.

**6.3. Read-After-Write (RAW) Hazard Resolution**
By bounding the maximum pipeline latency to a max FPU latency of 28 cycles, the architecture intrinsically resolves intra-thread RAW hazards:
...
**7.1. Pixel Snoop Buffer** (`pixel_buffer_ram.vhd`)
As the barrel scheduler issues a `RETURN` instruction over 32 cycles (one thread per cycle), `execution_unit` snoops the VRF read data at Stage 1 and provides three outputs: `mem_store_valid`, `mem_store_data` (4 × 32-bit XYZW components), and `mem_store_thread_id`. The warp_unit uses these to fill a 32-entry × 32-bit pixel ram buffer. Each entry packs the lower 8 bits of all four components into a single 32-bit RGBA word.
The full 1024-bit buffer is exposed as `pixel_buf_data` on the warp_unit port boundary.

**7.2. Block Transfer Memory Controller** (`mcu_block_transfer.vhd`)
...
**7.3. Avalon Burst Bridge** (`avm_burst_bridge.vhd`)
...
Simulated floating-point IPs are described in `fp_entities.vhd` and implemented in `fp_sim_arch.vhd`.
The latency values are chosen to match actual Altera floating-point IP values so they can be easily swapped during synthesis, simply by using `fp_ip_arch.vhd`.
A complete suite of testbenches can be found in `src/tb_*` and run using `make test-all`.
Furthermore, a set of automated tests that assembles real programs and loads them into a VHDL testbench can be run using `python tools/runner.py`, outputting the resulting `.png` images into the `tools/` directory.
